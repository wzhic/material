...
- 用户是需求范围、关键风险、验证豁免、不可逆操作、合并、部署和发布的唯一审核人；批准范围内的本地编辑、测试、受控提交和非强制功能分支 push 不逐项审批。
...
仓库已保留 G0 历史提交；当前 `GOV-0002` 在独立功能分支实施治理简化，尚未形成该任务的内容提交或 CI 证据。workflow 文件存在不表示当前任务的 GitHub CI 已运行；在真实 push、PR 和合并后 workflow 分别绑定准确提交并成功完成前，当前阶段证据保持 `PENDING`。私有仓库的 CI/合并后结果必须由 `taskctl sync-github-run` 使用仓库外 Actions 只读凭据在线查询 GitHub REST API；调用者自报的 URL、结论或日志片段不能形成 PASS。
...
空仓库首次推送的严格 A/B 协议只属于 GOV-0001 历史引导。后续普通任务在完整 local gate 后使用 `taskctl commit-task` 生成内容提交 A 与只含受保护状态的控制提交 B，再由 `taskctl push-task` 将当前功能分支非强制快进到固定仓库；直接 Git 写操作、空提交、强制推送、范围外路径、控制提交夹带普通内容或远端非祖先均被拒绝。提交后同范围 CI 失败使用 `recover-blocked` 归档旧证据并追加修复，不改写历史。
