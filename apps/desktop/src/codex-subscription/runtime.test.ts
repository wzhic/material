...
describe('Codex sidecar runtime', () => {
  ...
  it('reuses only the exact managed config and never overwrites an existing file', async () => {
    ...
    await expect(prepareCodexHome(requestedHome)).resolves.toBe(canonicalHome);
    expect(await readFile(configPath, 'utf8')).toBe(CODEX_APP_SERVER_CONFIG);

    await writeFile(configPath, 'user-owned-do-not-overwrite', { mode: 0o600 });
    await expect(prepareCodexHome(requestedHome))
      .rejects.toMatchObject({ code: 'SECURITY_VIOLATION' });
    expect(await readFile(configPath, 'utf8')).toBe('user-owned-do-not-overwrite');
  });
