...
export const prepareCodexHome = async (requestedPath: string): Promise<string> => {
  ...
  try {
    ...
    const temporaryConfigPath = path.join(
      canonicalHome,
      `.config.toml.${process.pid}.${Date.now()}.tmp`,
    );
    try {
      const handle = await open(temporaryConfigPath, 'wx', 0o600);
      try {
        await handle.writeFile(CODEX_APP_SERVER_CONFIG, { encoding: 'utf8' });
        await handle.chmod(0o600);
      } finally {
        await handle.close();
      }
      // Recheck the directories immediately before replacing the app-owned file.
      await assertRealDirectory(scopedRoot);
      await assertRealDirectory(absoluteHome);
      if (await realpath(scopedRoot) !== canonicalRoot
        || await realpath(absoluteHome) !== canonicalHome) {
        throw new CodexSubscriptionError('SECURITY_VIOLATION');
      }
      await rename(temporaryConfigPath, configPath);
      await chmod(configPath, 0o600);
    } finally {
      await rm(temporaryConfigPath, { force: true }).catch(() => undefined);
    }
