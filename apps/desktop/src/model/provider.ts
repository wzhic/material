...
export class OpenAiCompatibleProvider implements ModelProviderAdapter {
  ...
  async complete(
    ...
    try {
      ...
      const usageAvailable = isTokenCount(rawUsage.completion_tokens)
        && isTokenCount(rawUsage.prompt_tokens)
        && isTokenCount(rawUsage.total_tokens);
      const promptTokens = integerOrZero(rawUsage.prompt_tokens);
      const promptCacheHitTokens = Math.min(
        promptTokens,
        integerOrZero(rawUsage.prompt_cache_hit_tokens),
      );
      const usage: ModelUsage = {
        available: usageAvailable,
        completionTokens: integerOrZero(rawUsage.completion_tokens),
        promptCacheHitTokens,
        promptCacheMissTokens: isTokenCount(rawUsage.prompt_cache_miss_tokens)
...
export class OpenAiProvider extends OpenAiCompatibleProvider {
  ...
  override async complete(
    ...
    try {
      ...
      const usage: ModelUsage = {
        available: isTokenCount(rawUsage.input_tokens)
          && isTokenCount(rawUsage.output_tokens)
          && isTokenCount(rawUsage.total_tokens),
        completionTokens,
        promptCacheHitTokens,
