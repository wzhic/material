...
| 项目 | 内容 |
| --- | --- |
| 文档 / 版本 | Codex订阅模型分析-DEV-0020 / v1.0 |
...
| 任务 / 分支 | APP-0027 / codex/req-0008-app-0027-codex-analysis-provider |
...
目标是在既有 AnalysisEngine 的模型端口增加一个独立 Codex subscription provider，使同一份受控分析请求可由用户显式选择 API Key 或“Codex 订阅（Beta）”执行，同时保持既有编排、严格 schema、报告和历史生命周期。
...
- account/read 为已登录；
- rate limits 没有已知触限；
- model/list 当前目录包含所选 catalog ID，其 provider slug、default effort 和 text modality 合法；
...
system 内容来自版本化 Material Prompt，不接受用户任意系统指令；user 内容由 evidence packet 与产品快照确定性生成。任何额外消息、未知配置、过大 schema、不可序列化值、非法目录 ID/slug/effort 或 renderer effort override 在 thread/start 前返回 INVALID_INPUT。

## 8. 结构化文本证据

本地 EvidencePacket 先完成字段、上限、证据 ID 和去路径化校验，再渲染为受控文本。V1 允许 technical facts、采样时间、OCR/ASR 文本、视觉标签、产品字段、模板和规则摘要；禁止媒体字节、data URL、file URL、sourcePath、relocatedPath、工作区路径、凭据、其他记录和任意附件。
...
EvidencePacket 与输出均必须保留 evidenceId；模型输出只能引用输入集合内的 ID。输入正文不写入常规日志。

## 9. 一次临时 thread 与一次 turn

每次 complete 调用执行：

1. 创建随机空临时目录 material-codex-analysis-*。
2. 按当前权威目录的 catalog selection ID 解析 provider requested slug 和 frozen default effort；两者不从 renderer 接收。
3. `thread/start` 指定 `model=<provider slug>`、`config.model_reasoning_effort=<frozen effort>`、`allowProviderModelFallback=false`、ephemeral=true、本次空 cwd/runtimeWorkspaceRoots、approvalPolicy=never、sandbox=read-only、空 dynamicTools/environments/selected capability roots。锁定 `ThreadStartParams` 没有独立 effort 字段，所以 thread 级 effort 必须通过 config 发送；provider 不由 renderer 或 fallback 提供，而由响应校验必须为 OpenAI。
4. 在启动 turn 前逐字段校验 `ThreadStartResponse`：thread id 合法、thread.ephemeral=true、`modelProvider === "openai"`且 thread.modelProvider 一致、`model === provider requested slug`、`reasoningEffort === frozen effort`、`cwd` 与 `runtimeWorkspaceRoots` 只指向本次空目录、`approvalPolicy === "never"`、`multiAgentMode === "explicitRequestOnly"`、`sandbox.type === "readOnly"`、`sandbox.networkAccess === false`、`instructionSources` 为空，且可选 `activePermissionProfile` 只能是无 extends 的合法内建 profile。任一缺失/偏差先把可用的 provider returned slug 写入失败 audit，然后使当前 generation 失败关闭。
5. `turn/start` 只提交一条 text input、`model=<同一 provider slug>`、`effort=<同一 frozen effort>`、固定 outputSchema、summary=none、同一空 cwd/roots、approvalPolicy=never、readOnly + networkAccess=false。
6. 等待同一 thread/turn 的受控终态，收集最终文本、provider returned slug 与可用 token usage。
7. 结束后清理临时目录并移除活动上下文。
...
- `userMessage`：要求有界合法 `id`、可空有界 `clientId`、非空 `content`，且 V1 只接受由 `type=text`、有界 `text`、空 `text_elements` 组成的输入；校验后立即丢弃，不比较、回传或保存原文；
...
`userMessage` 与 `reasoning` 只为协议兼容，绝不进入 renderer、报告、日志或存储。上述任一类型缺少或畸形必填字段触发 PROTOCOL_ERROR；任何 tool、command、file、web、MCP、image、collab 或未知 item 触发安全失败。两者都使当前 generation 失败关闭。
...
thread 和 turn 的 cwd 都是新建空目录；结构化证据以内联 text input 提交，目录不包含素材、证据文件、路径映射或凭据。锁定 0.149.1 的 generate-ts schema 尚不支持实时官网新文档中的 restricted access/readableRoots，因此 V1 不声称协议强制文件根。
...
- 指令再次禁止 tools、files、network、apps、hooks、memories、skills 和 sub-agents；
...
- Request/envelope/schema 边界与恶意路径/媒体样本。
- 目录 ready/limited/换号/下线与显式来源路由；`id`/`model`/`defaultReasoningEffort` 缺失或非法 fail-close，同 slug 多 ID 不折叠，同 ID 冲突拒绝。
- 一次 thread/start、一次 turn/start、ephemeral、catalog selection ID 解析、provider slug + frozen default effort 在 thread config/turn 中一致、`allowProviderModelFallback=false`、空 cwd/roots。
...
- `Turn.items` 与 item 生命周期三类白名单：合法 `userMessage`/`reasoning` 校验后立即丢弃、合法 `agentMessage` 是唯一最终正文候选；三类必填字段畸形为协议失败，tool/command/file/web/MCP/image/collab/未知 item 为安全失败，均不得进入 renderer/报告/日志/存储。
...
| 固定 runtime 未登录 | initialize、版本、架构、未登录 account/read、安全退出 |
| 真实订阅开发环境 | account/model/rate-limit、一次受限分析、catalog/requested/returned/effort 与 usage 摘要；另行记录非代码工作流的产品/条款适配确认状态 |
...
真实订阅 smoke 除成功结果外还必须证明本次一个 ephemeral thread/turn 的 tool call 与 unexpected server request 计数均为 0；只检查最终文本而不检查事件流不算安全证据。
...
- 当前未执行真实 ChatGPT 订阅登录 + 目录 + 结构化分析 smoke；OpenAI/DeepSeek API Key live credential smoke 也因未提供凭据而未运行。上述未执行项不能记为“真实账号已调通”。
