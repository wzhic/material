...
| 文档状态 | DRAFT；作为 APP-0027 的订阅接入前置合同导入，真实订阅与双平台证据仍待补齐 |
| 交互类型 | mixed |
| 提出人 | 产品负责人 |
| 编写负责人 | Codex |
| 审核人 | 产品负责人 |
| 创建 / 更新日期 | 2026-08-25 / 2026-08-26 |
| 关联产品需求 | [单素材分析-REQ-0005-v1.0](../单素材分析-REQ-0005/单素材分析-REQ-0005-v1.0.md)；[Codex订阅模型分析-REQ-0008-v1.0](../Codex订阅模型分析-REQ-0008/Codex订阅模型分析-REQ-0008-v1.0.md) |
...
| 关联任务 | 来源 APP-0022；当前整合与分析接入 APP-0027 |
| 工作分支 | `codex/req-0008-app-0027-codex-analysis-provider` |
| base branch | `main` 基线 `c61d9e8ec74e684a1c717e9436c642f7c7c657b2`；导入来源分支 `codex/req-0007-app-0022-codex-subscription` |
| 发布单元 | macOS 客户端、Windows 客户端；不新增项目自建 backend |
| 上一版本 | 无，首次形成 Codex 订阅独立需求 |
| 变更摘要 | 从 APP-0022 导入订阅接入前置合同和两张 1280×800 原型，校正编号冲突与追踪链接；完整分析由 REQ-0008 定义 |
...
既有模型管理路径已建立 DeepSeek、官方 OpenAI API Key 和自定义 OpenAI 兼容 API 的模型配置、模型列表和显式固定测试。该路径属于 BYOK：API Key 的用量按对应 API 账号计费，不能直接消费用户的 ChatGPT 订阅权益。
...
官方 [Codex SDK](https://learn.chatgpt.com/docs/codex-sdk) 可在 Node.js 应用中启动和运行本地 Codex 线程，但账号登录、`model/list`、`account/rateLimits/read`、事件通知和退出等深度产品集成由 [Codex App Server](https://learn.chatgpt.com/docs/app-server) 提供。因此本需求以 App Server 为账号、目录、限额和测试的主接口，并精确锁定 `@openai/codex-sdk` `0.149.1` 所带官方 Codex runtime，避免依赖用户机器上另装或版本漂移的全局 CLI。官方同页将 SDK 定位为用于“coding-focused Codex threads”；Material 的素材分析是非代码工作流，技术上可通过受限 thread 接入，但不据此宣称已符合产品定位或条款；适用性需在真实账号 smoke 与发布审核中确认。

App Server 属 Experimental。实时官网与锁定 `0.149.1` runtime 的 generate-ts schema 可能存在版本差异；发布合同以锁定生成协议和 runtime 实测为准。官网新版本出现但 locked schema 尚无的 restricted access/readableRoots 等字段不得被写成当前已强制能力。

当前事实与声明边界如下：

- 本文档来自 APP-0022 的订阅接入基线；APP-0027 在其上定义分析接入，API Key 路径必须保持可用且不被订阅状态影响。
- REQ-0007 这份前置合同本身只定义订阅登录、账号与限额摘要、模型目录、显式选择和固定非业务测试。
- APP-0027 / REQ-0008 已在独立合同下把合格 Codex 目录项加入“新建分析”候选，并单独定义结构化证据、产品数据、Prompt、报告和 PDF 边界；不得从 REQ-0007 单独推导任意业务输入权限。
...
本文档的前置接入能力最多交付前五项中的已实际运行证据，第六项由 REQ-0008 和 APP-0027 单独验收。任何 `SKIP`、未登录 runtime、mock 成功或单平台结果都不能升级为更高层结论。
...
- REQ-0007、DEV-0020、TRB-0017、需求/开发/排障索引和两张原型截图映射。

### 7.2 排除范围

- REQ-0005 的完整分析工作区、Prompt/证据组装、报告与记录 schema。
- 既有 API Key 凭据协议改造；仅要求其行为不退化。
...
来源 APP-0022 的前置能力与 APP-0027 的分析接入以各自机器任务清单为准；当前文档追踪至 REQ-0007、REQ-0008、DEV-0020、TRB-0017 及对应索引。macOS 与 Windows 是两个独立验收单元；不新增 backend 发布单元。
...
| CS-A-02 | 已确认 | 范围 | 本前置合同只交付登录、目录、限额、选择和固定测试；分析由 REQ-0008 接续 | 不完整分析属于预期，不把测试成功包装成业务完成 | 可逆；任务范围与用户要求拆分 |
...
| CS-A-08 | 已确认 | 目录 / 推理 | `model/list.id` 是目录/预设选项 ID，`model/list.model` 是实际请求 slug；V1 不提供 reasoning effort 切换 | 同 slug 多预设必须保留为多个选项，并冻结所选项的 `defaultReasoningEffort` | 可逆；锁定 `0.149.1 generate-ts --experimental` 和官方 App Server 目录合同 |
...
| CS-Q-05 | Codex 官方主要面向 coding-focused threads，非代码素材分析的产品/条款适配性 | 仅 Beta，API Key 保持可用且不设 Codex 为默认 | 真实账号 smoke 和发布审核前 | 不作为默认/稳定来源，必要时停用 Codex 候选；不引申为条款禁止 |
...
2. 页面标题为“模型接入”，副标题明确“订阅与 API Key”；同一页面按视觉和语义分成：
   - 第一分区“Codex 订阅（Beta）”；
   - 第二分区“API Key 模型”。
3. Codex 分区不是 BYOK 表单，不显示 API Key、Base URL 或自定义模型 ID；API Key 分区继续遵循既有模型管理合同，订阅状态不改变其保存、刷新、测试和删除。
...
5. 若客户端未启用 REQ-0008 分析 provider，Codex 不进入新建分析候选，并显示“当前仅支持登录、模型发现和连通测试”。当 REQ-0008 在同版本启用时，不再显示“尚未接入”，只将符合该需求的 ready 目录项发布为显式候选；仍不自动开始分析。
...
6. reset credits 只显示 App Server 返回的 `availableCount` 或“暂不可用”。Material 不兑换 credits、不购买、不发送加额邮件、不硬编码价格或模型 credits 费率。
...
2. 锁定 `0.149.1 generate-ts --experimental` 的 `Model` 同时包含 `id`、`model` 和必填 `defaultReasoningEffort`：`id` 是目录/预设选项 ID，`model` 是发给 provider 的实际请求 slug；两者不得混用。
3. 游标合并只能按 `id` 判定同一目录项。多个目录/预设项可以共享同一 `model` slug，必须分别保留、分别可选，不得按 slug 折叠、去重或用后一项覆盖前一项。
4. 保存非秘密的 `id`、`model`、显示名、`defaultReasoningEffort`、支持的 effort 摘要、默认标志和输入模态。缺少/非法 `id`、`model` 或 `defaultReasoningEffort`、同 ID 内容冲突、非法/超限响应均失败关闭；未知可选字段忽略。
5. 只显示支持文本输入的可见目录项；服务标记 `isDefault` 仅用于“推荐”提示与排序，不自动替用户选择。
6. V1 不显示 reasoning effort 选择器，不允许 renderer 传 effort override。用户选择一个目录项时，同时冻结该项的 `id`、`model` slug 和 `defaultReasoningEffort`；同 slug 多预设通过选项 ID 与对应默认 effort 保持区分。
7. 用户必须显式选择目录项；选择值必须精确来自最新成功目录的 `id`。手填、历史残留或其他账号的 ID/slug/effort 组合在联网前拒绝。
8. 选择只保存非秘密 `selectedModelId`，其语义固定为目录/预设 `id`，不是 provider slug；账号退出、账号身份变化或目录刷新后不再包含该 ID 时，旧 ID 失效并清空可调用选择，页面说明“原模型不可用，请重新选择”。
9. 模型下线、隐藏、默认 effort 变化或服务推荐升级都不触发运行中自动切换；新请求发现目录契约变化时清空旧选择，用户重新选择前测试按钮禁用。
10. 目录失败时可保留上一份列表作为“上次快照”说明，但不得用于新选择或测试；恢复网络后由用户刷新。
...
4. 主进程使用版本固定的短文本创建一个隔离、非持久的测试 thread：`thread/start.model` 发冻结的 provider `model` slug，`thread/start.config.model_reasoning_effort` 发冻结的 `defaultReasoningEffort`，且 `allowProviderModelFallback=false`；随后唯一的 `turn/start` 再显式发同一 slug 和 effort。Material 不在失败、断流或重连后创建第二个 turn，不续跑、不换目录预设/模型/effort，不回退 API Key。
...
6. `Turn.items` 与 `item/started` / `item/completed` 只在锁定协议必填字段、类型和长度严格合法后识别 `userMessage`、`reasoning`、`agentMessage` 三类非工具 item；`userMessage` 与 `reasoning` 仅作协议兼容并立即丢弃，只有合法 `agentMessage` 可作为最终正文候选。任一畸形 item、未知 item 或 tool/command/file/web/MCP/image/collab item 都使当前 generation 失败关闭。
7. 成功只返回 checkedAt、duration、catalogSelectionId、providerRequestedModelSlug、providerReturnedModelSlug、frozenReasoningEffort 和 plan 摘要；模型正文、reasoning、测试文本、thread/turn ID、原始 usage、JSONL 和 stderr 不进入 renderer 或持久化。
8. 成功只证明该账号/模型当次固定测试可达，不代表模型质量、剩余额度、完整素材分析或发布验收。

### 10.10 工具、文件和扩展能力禁用（CS-FR-13）

1. 测试工作目录是应用创建的空目录，不是仓库、素材目录、用户主目录、Desktop、Documents 或全局 Codex 目录。
2. 结构化固定测试不附加目录或文件；锁定协议不能按 readableRoots 强制可读根，因此不以“runtime 进程绝对不可读其他路径”作为已实现声明。
3. app-scoped strict config 主动关闭 shell/command、unified_exec、file change/apply patch、computer use、view image、web search、network command、MCP、apps/connectors、dynamic tools、hooks、skills、plugins、memory、AGENTS、rules 和子 Agent；thread/turn 均 read-only，turn network off，工具/environment/capability 列表为空。pinned 0.149.1 实测接受 `[features] view_image=false/shell_tool=false/unified_exec=false` 与根级 `web_search="disabled"`，并拒绝实时官网新式 `[tools] view_image=false`；生产配置必须服从锁定 strict schema。
4. `approvalPolicy = never` 不代表许可；任何 command/file/tool/MCP/web/server request 或 approval 事件都视为安全违规，使同一 runtime generation 失败关闭、interrupt 并返回 `SECURITY_VIOLATION`。
...
- 任何权限、工具或要求用户追加输入的 server request/elicitation 一律拒绝；不得显示“允许本次”或“总是允许”。这不包括锁定协议在当前 turn 生命周期内回显的合法 `userMessage` item；该 item 只经严格校验后立即丢弃。
...
未在白名单中的客户端发起方法禁止调用。锁定 `0.149.1 generate-ts --experimental` 的 `Turn.items` 和 item 生命周期通知仅允许三类严格合法的非工具 item：`userMessage`（仅协议兼容，立即丢弃）、`reasoning`（仅协议兼容，立即丢弃）和 `agentMessage`（唯一可作为最终正文候选）；前两类绝不进入 renderer、报告、日志或存储。任何 approval、要求追加用户输入的 server request/elicitation、tool、command、file、MCP、web、dynamic tool、image view、collab、未知 item，或上述三类 item 的畸形必填字段，都必须使当前 generation 失败关闭并中断测试。
...
- `ErrorNotification.willRetry` 的 `true` 和 `false` 都是锁定协议的合法布尔值；Material 收到任一值都立即终止本次测试，不等待其声明的后续恢复、不创建第二个 turn，也不在应用层重试或回退。
...
上述约束只控制 Material 到 App Server 的逻辑请求数。Codex 内置 OpenAI Provider 的 `request_max_retries` / `stream_max_retries` 有上游配置语义，固定 runtime 可能在同一个 turn 内执行传输恢复；实现和验收因此只承诺“一次确认对应一个 thread + 一个 turn、客户端不二次发起”，不承诺单次 HTTP 尝试。依据见官方 [Config Reference](https://learn.chatgpt.com/docs/config-file/config-reference)，具体 locked 行为以固定 runtime 协议测试为准。该行为必须进入测试前额度披露、Beta 风险和每次 runtime 升级门禁。
...
- 自动化不消耗真实订阅；真实 smoke 需要用户/测试负责人明确登录和触发，每次只运行一条固定测试。
...
| ID | 要求 | 可验证边界 |
| --- | --- | --- |
| CS-NFR-01 安全与秘密禁入 | token、原始账号、动态 auth URL、Prompt/回复/reasoning 不进入 renderer、日志或业务存储；仅当前设备码登录 dialog 可瞬时接收一次性 `userCode`，用户主动复制，终态清除且不得残留到其他 renderer 状态、日志、持久化、报告或崩溃信息；全局 Codex 状态不读取 | 秘密哨兵、IPC/DOM/日志/产物扫描和双平台真机隔离 |
...
- 排障按 [Codex订阅模型分析-TRB-0017-v1.0](../../troubleshooting/Codex订阅模型分析-TRB-0017-v1.0.md) 执行；不得要求用户提供 token、完整 auth URL、设备码或 `auth.json`。
...
1. APP-0027 不修改 API Key Vault schema，不解密、迁移或重写现有 Key。
...
| CS-AC-03 | 在 API Key 区执行既有新增、刷新、测试和删除 | 既有 API Key 行为不退化；订阅状态不参与其鉴权、模型或错误 |
| CS-AC-04 | 移除/损坏 bundled runtime 或令 keyring 不可用 | Codex 区显示 `unavailable` 和恢复动作；API Key 区仍可用且无文件凭据回退 |
| CS-AC-05 | Codex 测试成功后打开新建分析 | 未启用 REQ-0008 时不出现订阅候选并说明仅前置能力；同版本启用 REQ-0008 时，合格 ready 目录项可作为显式候选，但测试成功不自动选择或开始分析 |
...
| CS-AC-23 | `model/list` 返回多页合法目录，包含多个共享同一 `model` slug 的预设项 | 客户端固定 `includeHidden:false`、按 cursor 合并，只按目录 `id` 判定同一项；同 slug 的不同 ID 全部保留且不相互覆盖 |
| CS-AC-24 | 返回 hidden、无 text 模态、缺 `id`/`model`/`defaultReasoningEffort`、非法项、空目录、同 ID 冲突或超过 200 个唯一有效项 | hidden 与不支持文本项不展示；必需字段缺失/非法、同 ID 冲突和超上限失败关闭；过滤后为空进入“无模型”状态，不伪造模型或 effort |
| CS-AC-25 | 首次目录成功但用户未选目录项 | `isDefault` 只作推荐，测试禁用且要求用户显式选择；V1 无独立 reasoning effort 选择器 |
| CS-AC-26 | 选择目录内/目录外项，并注入伪造 slug 或 effort | 只保存目录 `id`，开始时从同一权威项解析并冻结 provider slug 与默认 effort；手填或旧账号组合在上游请求前拒绝 |
| CS-AC-27 | 刷新后原目录项下线、隐藏、更换 slug/default effort 或出现 upgrade 建议 | 原选择失效，测试禁用，用户必须重选；不自动采用另一预设、effort、推荐或升级模型 |
| CS-AC-28 | 目录刷新失败但存在旧快照 | 旧快照标记过期，只可说明，不可新选或测试；账号和 API Key 配置保留 |
| CS-AC-29 | 退出后用另一账号登录 | 旧账号目录、限额和选择已清空；新目录重新加载并要求选择 |
| CS-AC-30 | 已就绪时点击测试 | 确认弹窗准确显示账号、plan、目录选项/模型摘要、固定测试边界、额度/credits 和同 turn 内可能发生的 runtime 传输恢复；点取消只关闭确认且不启动测试 |
...
| CS-AC-34 | 一次点击期间重复点击、网络失败、断流、模型失败，或分别注入 `ErrorNotification.willRetry=true/false` | App Server 侧观察到最多一次 `thread/start` 与一次 `turn/start`；两种 `willRetry` 都终止 Material 本次测试，客户端不等待重试、不发第二 turn、不续跑或切换。固定 runtime 在错误通知前同一 turn 内可能执行的传输恢复仍按已披露边界处理 |
| CS-AC-35 | 触发 60 秒截止并制造 turn 终态同时到达 | 主进程最多发送一次 interrupt，以先发生的受控终态结束，不产生第二个 turn；页面没有运行中手动取消入口 |
| CS-AC-36 | 在 `Turn.items` 和 item 通知中返回严格合法的 `userMessage`、`reasoning`、`agentMessage`，以及 usage、thread/turn ID 和模型元数据 | 前两类 item 校验后立即丢弃且四类禁止位置命中数为 0；只有 `agentMessage` 可作为最终正文候选，页面仍只得到时间、耗时、catalog selection ID、provider requested/returned slug、冻结 effort 和 plan 摘要 |
| CS-AC-37 | 注入 `ThreadStartResponse` 的 provider、slug、reasoningEffort、cwd/roots、approval 或 readOnly/network 偏差 | 同时保留目录选择 ID 和 requested/returned slug，当前 generation 失败关闭；不覆盖、不称为自动切换，不用第二个预设/模型/effort 重试 |
| CS-AC-38 | 测试成功前后检查账号、目录、限额和新建分析 | 成功仅为瞬时摘要，不改目录/选择、不自动刷新额度、不授权完整分析 |
| CS-AC-39 | 给 app-scoped config、空 cwd 和工具面设置 canary | cwd 仅为受控空目录、无 additionalDirectories；strict config 关闭所有文件/命令能力，工具列表为空；不声称 locked schema 尚无的 readableRoots 强制 |
...
| CS-AC-44 | sidecar 输出恶意/含哨兵 stderr、超长或非法 JSONL，或三类允许 item 缺少/畸形必填字段、出现未知 item | 当前 generation 失败关闭并只得到稳定错误；renderer、console、文件日志和产物无原始内容 |
...
| CS-AC-68 | 依次执行 mock、无登录 runtime 和真实专用账号 smoke | 三类证据分开：mock/无登录不算真实调通；真实 smoke 同会话验证账号、目录、限额、一次测试以及 tool/unexpected server request 计数均为 0，且只证明当次可达 |
...
| 受控汇总 | 由主任务通过治理工具执行 APP-0027 的 required validations | 任务声明的全部 required checks | 只有实际退出码/结果可生成 PASS；本文档不代运行 |
...
- model/list 游标、hidden、text modality、默认推荐、`id`/`model`/`defaultReasoningEffort` 必需性、同 slug 多 ID 不折叠、非法项、空目录、超限、选择下线和账号切换；
- 固定测试 gate、目录 ID 解析为 provider slug + 冻结默认 effort、thread config/turn effort 一致、`ThreadStartResponse` 全字段校验、单 thread/turn、60 秒截止 interrupt、无运行中手动取消 API、`ErrorNotification.willRetry` 两种布尔值都终止且无客户端二次 `turn/start`、同 turn 内传输恢复披露、catalog/requested/returned/effort 审计；严格合法的 `userMessage`/`reasoning` 校验后丢弃、仅 `agentMessage` 可作最终正文，三者畸形均失败关闭；
- command/file/permission/MCP/web/dynamic tool/image/collab 及 unexpected server request 同 generation fail closed；
- app-scoped strict config、环境白名单、空 cwd、无额外目录、锁定 generate-ts schema、全局配置 canary 和无持久 prompt/rollout；
...
2. 同一 runtime 会话依次核对 `account/read.account` 是 ChatGPT 账号对象、`model/list`、`account/rateLimits/read` 和一个用户显式选择目录 ID 的固定测试；证据保留该 ID、provider requested/returned slug 与冻结 effort；
...
| [Codex订阅模型分析-DEV-0020-v1.0](../../development/Codex订阅模型分析-DEV-0020-v1.0.md) | 承接 App Server/JSONL/sidecar/IPC/隔离，并补充完整分析接入设计 |
| `docs/development/README.md` | 增加 DEV-0020 索引；来源 DEV-0013 不导入，避免与现有编号冲突 |
| [Codex订阅模型分析-TRB-0017-v1.0](../../troubleshooting/Codex订阅模型分析-TRB-0017-v1.0.md) | 新增登录、runtime、额度、模型、分析、取消与安全失败恢复 |
| `docs/troubleshooting/README.md` | 增加 TRB-0017 索引；来源 TRB-0010 不导入，避免与现有编号冲突 |
| REQ-0005 / REQ-0008 / DEV-0010 / TRB-0007 | 订阅与 API Key 保持独立；完整分析、报告与记录由 REQ-0008 接续 |
...
| Beta 被当成生产完成 | 用户发送真实业务数据或依赖不稳定能力 | 前置/分析需求分层、REQ-0008 候选门禁、证据层级、真实账号与适配性发布门禁 | 移除 Beta 必须另行决定，不能由单次 smoke 自动触发 |
| SDK 官方定位与素材分析适用性不确定 | 非代码工作流超出 coding-focused 主要定位，后续产品或条款要求与假设不一致 | 仅 Beta、不默认选中、保留 API Key、真实账号 smoke 和发布审核 | 官方文档未在本需求中被解读为“禁止”；适用性未确认前不得作为默认/稳定来源 |

本需求新增账号、外部数据处理与可能额度消耗，但用户已明确要求使用自己的 Codex 订阅。该范围决定不等于授权读取全局 token、自动消耗真实额度、完整素材外发、最终合并、部署或发布。

## 23. 审核与回执

- 用户在当前对话明确提出“支持接入 Codex SDK，支持直接使用我的 Codex 订阅”，构成来源 APP-0022 的产品范围决定，并由 APP-0027 接续分析接入。
- 用户此前要求“测试模型是否能够调通”支持本需求设计显式固定 smoke，但实际 ChatGPT 登录和每一次真实额度调用仍必须由用户在应用界面主动完成；不得复用其全局登录自动测试。
- 来源任务摘要已确认：订阅与 API Key 独立、app-scoped home、固定测试不发送素材、界面不承诺无限用量；完整分析另按 REQ-0008 受限证据合同执行。
...
| v1.0-import | 2026-08-26 | 导入 APP-0022 文档和两张原型，校正 REQ/DEV/TRB 追踪编号，并链接 REQ-0008 完整分析合同 | 合并后现有 DEV-0013/TRB-0010 已被其他能力占用，必须避免编号冲突 | APP-0027 |
