# Codex订阅模型分析-REQ-0008-v1.0

## 1. 文档信息

| 项目 | 内容 |
| --- | --- |
| 需求名称 / 编号 / 版本 | Codex 订阅模型分析 / Codex订阅模型分析-REQ-0008 / v1.0 |
| 文档状态 | DRAFT；需求合同完整，真实订阅、签名安装包和双平台证据待执行 |
| 交互类型 | mixed |
| 提出人 / 审核人 | 产品负责人 / 产品负责人 |
| 编写负责人 | Codex |
| 创建 / 更新日期 | 2026-08-26 / 2026-08-26 |
...
| 关联任务 / 分支 | APP-0027 / codex/req-0008-app-0027-codex-analysis-provider |
...
用户可在新建分析时显式选择“Codex 订阅（Beta）”中的文本模型，应用仅发送由本地解析产生的受限结构化文本证据，通过一次新的临时线程和一次逻辑 turn 获得结构化分析结果；订阅失败、限额、离线或取消均失败关闭，不自动重试、切换模型或回退 API Key。
...
- [Codex App Server](https://learn.chatgpt.com/docs/app-server) 提供账号、模型、限额、线程、turn 和事件接口；App Server 整体及部分传输/方法存在 Experimental 边界，不承诺生产稳定。
...
2. 用户确认本次会消耗订阅额度，且只发送结构化文本证据后开始分析。
...
| 外发最小化 | 只外发结构化文本证据，不发送媒体字节或本地路径 | CA-FR-10～CA-FR-13，CA-AC-17～CA-AC-24 |
...
- 不发送原始图片、视频、音频、缩略图、二进制附件、媒体 URL 或本地绝对/相对路径。
...
- Codex 文本模型候选、显式选择、启动确认、状态、取消、超时、失败和成功。
- 受限结构化证据 envelope、严格结构化输出 schema、报告与历史记录字段。
- 应用专属登录、固定 runtime、空临时工作目录、只读 sandbox、无工具网络和秘密隔离。
...
| 编号 | 类型 / 状态 | 内容 | 处理 |
| --- | --- | --- | --- |
| CA-A-01 | 已确认 | V1 只支持文本模型分析 | 非文本目录项不进入候选 |
...
| CA-A-03 | 已确认 | 只发送本地生成的结构化文本证据 | 媒体和路径进入 envelope 前即拒绝 |
...
| CA-FR-03 | 只有 account ready、目录已加载、非 limited，且 `id`、provider `model` slug、`defaultReasoningEffort` 和 text modality 均合法的目录项可成为 Codex 候选；缺任一必需字段失败关闭。 |
...
| CA-FR-10 | 启动前必须再次说明“会消耗订阅额度”和“V1 只发送结构化文本证据”，用户可取消。 |
...
| CA-FR-12 | thread 使用应用新建的空临时 cwd，read-only sandbox 和 app-scoped strict config；锁定 runtime 协议尚不能证明按 readableRoots 强制文件根，因此 V1 以禁用全部文件/命令工具、空 cwd、无业务文件和意外请求同 generation 失败关闭实现边界。 |
| CA-FR-13 | 输入只能是有版本、有限长、可验证的结构化文本 envelope；媒体字节、URL、本地路径和未知字段失败关闭。 |
| CA-FR-14 | `Turn.items` 与 item 生命周期只在锁定协议必填字段、类型和长度严格合法后识别 `userMessage`、`reasoning`、`agentMessage` 三类非工具 item；只有合法 `agentMessage` 可作为最终正文候选，且仍须符合固定 JSON Schema。自由文本、工具结果、代码块包裹、缺字段或超限结果不得进入报告。 |
| CA-FR-15 | 合法 `userMessage` 与 `reasoning` 仅作协议兼容并立即丢弃，绝不进入 renderer、报告、日志或存储；进度只暴露 Material 定义的稳定阶段。任一允许类型的畸形必填字段、未知 item，或 tool/command/file/web/MCP/image/collab item 都使当前 generation 失败关闭。 |
...
登录、账号、模型目录和限额遵循 REQ-0007：ChatGPT 托管登录只存在于 Material 专属 CODEX_HOME/keyring，API Key 走独立存储合同；renderer 不接收任何 token、动态 auth URL 或通用 RPC。唯一例外是设备码登录当前尝试的固定验证 URL 与一次性 `userCode` 可在当前 dialog 瞬时展示，只允许用户主动复制，并在尝试终态、取消或 dialog 关闭后清除；不得进入其他 renderer 状态或留下 DOM 残留。分析运行没有 shell、文件、工具或额外网络批准入口。

### 14.1 允许外发

只允许由本地确定性解析与规则层形成的结构化文本：
...
报告、确认后记录与 PDF 只保存业务结果、来源类型、catalog selection ID、provider requested/returned slug、冻结 reasoning effort、非秘密 runtime 版本、usage 和追踪 ID。临时 thread id 仅在本次主进程生命周期内用于取消/关联，不作为可恢复聊天保存；临时 cwd 和受限证据文件在受控终态后清理。清理失败记录非秘密告警，不扩大删除目标。旧记录上述新字段可缺失；阅读器显示“暂不可用”而不猜测。
...
- 必须逐字段校验 `ThreadStartResponse`：`modelProvider === "openai"`、返回 `model` 等于冻结 provider slug、`reasoningEffort` 等于冻结 effort、`cwd` 和 `runtimeWorkspaceRoots` 等于本次空临时目录、`approvalPolicy === "never"`，且 `sandbox.type === "readOnly"` 与 `networkAccess === false`。任一偏差都保留非秘密请求/返回摘要并使当前 generation 失败关闭。
- cwd 指向新建空临时目录，不指向素材目录、仓库或用户目录。
- 结构化文本证据直接作为 text input 提交，临时 cwd 内不落原始媒体、证据文件、路径映射或凭据。
...
- `Turn.items`、`item/started` 与 `item/completed` 共用严格 item 合同：只识别合法 `userMessage`、`reasoning`、`agentMessage`；前两类验证后立即丢弃，只有 `agentMessage` 是最终正文候选。允许类型缺少或畸形必填字段按协议失败关闭；tool/command/file/web/MCP/image/collab/未知 item 按安全边界使同一 generation 失败关闭。
...
| 编号 | 需求 |
| --- | --- |
| CA-NFR-01 | 安全：renderer 不接收 token、动态 auth URL、原始 App Server RPC、文件路径或证据全文日志。仅设备码登录当前尝试的一次性 `userCode` 可在当前 dialog 瞬时展示并由用户主动复制；终态后清除，且不得进入其他 renderer 状态、DOM 残留、日志、持久化、报告或崩溃信息。 |
| CA-NFR-02 | 最小权限：主进程只暴露固定 DTO 和动作白名单；严格合法的 `userMessage`/`reasoning` 仅在主进程瞬时校验后丢弃，所有未知/畸形 item 与工具请求失败关闭。 |
...
| CA-AC-17 | 点击开始 | 先显示订阅额度和结构化文本外发提示 |
| CA-AC-18 | 在确认框取消或按 Escape | 不创建 thread，不启动 turn |
| CA-AC-19 | 检查输入 envelope | 只有允许的结构化文本字段与有限长度 |
| CA-AC-20 | 注入媒体字节、URL或任一路径字段 | 启动前失败关闭 |
| CA-AC-21 | 成功开始一次分析 | 创建一个新 ephemeral thread 和一个逻辑 turn；thread/start 发 provider slug + config effort + `allowProviderModelFallback=false`，turn/start 发同 slug + effort |
| CA-AC-22 | 检查 `ThreadStartResponse`、cwd、strict config 与锁定协议 | 逐字段等于 `modelProvider=openai`、冻结 slug/effort、空 cwd/roots、approval never、readOnly + network false；任一偏差失败关闭。工具全禁用、证据只以内联文本提交，不虚构 locked schema 尚无的 readableRoots 强制 |
| CA-AC-23 | 模型尝试文件/命令/MCP/web/network tool | 立即安全失败并取消，不展示或保存结果 |
| CA-AC-24 | 检查分析主进程到 renderer DTO | 无 token、动态 auth URL、设备码 `userCode`、路径、原始 RPC 或证据正文；登录 DTO 的瞬时 `userCode` 例外严格受 CA-NFR-01 限制 |
| CA-AC-25 | 分别在 `Turn.items`、`item/started`、`item/completed` 注入严格合法的 `userMessage`、`reasoning`、`agentMessage` | 前两类只作协议兼容并立即丢弃，renderer/报告/日志/存储均无命中；只有 `agentMessage` 可作为最终正文候选，UI 仍只显示稳定阶段 |
...
| CA-AC-30 | 模型拒绝、返回非法 schema、允许类型 item 的必填字段畸形，或返回 tool/command/file/web/MCP/image/collab/未知 item | 当前 generation 失败关闭，不追加修复 turn，不保存部分报告；工具/未知 item 无执行或副作用 |
...
| CA-AC-44 | 扫描业务存储和日志 | 无 token、设备码、auth URL、媒体路径、prompt 或证据正文 |
| CA-AC-45 | 仅 mock 测试通过 | 结论只写“协议/状态机通过”，不写真实订阅可用 |
| CA-AC-46 | 固定 runtime 未登录启动成功 | 结论只写“runtime 可启动”，不写订阅已调通 |
| CA-AC-47 | 专用账号完成真实 smoke | 记录账号类型、catalog selection ID、provider requested/returned slug、冻结 effort、usage/缺失、一次 thread/turn、tool/unexpected server request 计数均为 0 和时间，不扩大为签名包证据 |
...
| 安全 | 媒体/路径拒绝、三类非工具 item 严格校验与丢弃边界、工具/未知 item/server request fail-close、`ErrorNotification.willRetry` 双值终止、秘密扫描 | 零泄露、零副作用和零第二 turn |
| 真实订阅 | 用户主动执行一次 smoke | catalog selection ID、provider requested/returned slug、冻结 effort、usage/缺失、当时限额和结果 |
...
| 文件/工具能力误启 | 本地数据读取或副作用 | 空 cwd、strict config、read-only、工具全禁、意外请求 fail-close；locked schema 无 readableRoots 强制 |
...
- V1 只发送结构化文本证据，不发送原始媒体或路径。
- 一次分析使用一个新 ephemeral thread 和一个逻辑 turn；空临时 cwd、read-only、strict app-scoped config、无工具和无工具网络；锁定协议不声称具有 readableRoots 强制。
