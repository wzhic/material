...
- 用户：唯一关键决策人，在 Codex 对话中确认需求范围、依赖/重大架构、数据安全与不兼容迁移、验证豁免、不可逆操作、合并、部署和发布；范围内本地步骤不逐项审批。
...
stateDiagram-v2
    ...
    LOCAL_VERIFIED --> COMMITTED: taskctl commit-task 记录内容 A / 控制 B
...
流程图不能代替异常规则：关键节点缺明确用户决定、R003 迁移根损坏、范围变化、错误分支、状态损坏、runner 改动工作区、GitHub 网络/凭据缺失、事件 SHA 不可信、测试未运行或证据阶段不符都必须进入阻断，而不是沿成功路径继续。需求范围、验证豁免、不可逆操作或合并/部署/发布缺回执时，Codex 等待用户在当前对话明确决定；普通范围内本地步骤不重复索取回执。
...
| v1.0 治理修订 | 2026-08-19 | 将普通任务调整为范围内持续本地工作、受控 A/B 提交、非强制功能分支 push 与无重复确认的同范围返工；关键节点审批保持不变 | 降低单人维护摩擦，同时保留验证、范围和不可逆边界 | GOV-0002 / 用户 |
