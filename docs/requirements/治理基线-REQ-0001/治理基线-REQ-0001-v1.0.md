...
| 状态 | IN_PROGRESS；R003 最终签名迁移已完成，R004 已批准 scope v4，R005/R006 已批准首次 CI 闭环与受控提交传输，R007/R008 分别批准两次真实 Windows CI 失败的精确非强制恢复；正在重新执行全部本地验证，不表示 CI 已通过或任务已完成 |
...
本需求只允许建立 `.codex/` Hook、`tools/governance/` 控制工具、`project-control/` 机器记录、分维度文档和 `.github/` 治理 CI。它不授权应用脚手架、应用依赖、业务代码、产品数据库迁移、合并或部署；治理快照的提交与 push 只能走 R006-R008 约束的 `taskctl` 窄入口。
...
首次 G0 push 的闭环固定为：根内容提交 A 在 `GOV-0001/main` 上运行三个发布单元的全部批准检查并核对完整根快照，但只返回 `bootstrap_pending`、不写 CI PASS。Run `32221854668` 的 Windows job 因 CP1252 解码 Git UTF-8 输出失败；历史 A 保留，R007 只允许 A 的固定消息、精确路径非强制子提交 C `7b18bb4c0b3cc42e078acf5351a19f6760beea0f`，其 CI 返回 `bootstrap_repair_pending`。Run `32223213198` 证明 Git 解码已修复，但 C 又暴露 CP1252 标准输出和无 HOME Git 发现失败；R008 只允许 C 的固定消息、精确路径非强制子提交 D，D 的首次 CI 仍为 PENDING。D 的三个 job 全部成功后，`taskctl` 才记录 D，只形成受保护控制提交；控制提交再次全量运行，证明 D 是控制 head 的祖先且区间全为受保护机器记录后，才报告 `bootstrap_control_head` 并允许 GitHub REST 同步。项目 bootstrap 白名单必须精确只有 `GOV-0001`；其他任务、第三个修复、普通内容进入控制区间、空提交、强制推送或跳过检查均拒绝。
...
| AC-11 | push 或发起 PR | CI 用事件 base/head 核对已提交范围；PR detached HEAD 使用可信事件分支；首次 G0 内容 run 与精确 R007/R008 修复 run 只产生 PENDING，受保护控制 head 全量复验后才可同步 PASS | Run 32221854668、32223213198 已保留为 Windows 失败证据；待 R008/D run |
...
- R001、R002、R003 保留为历史迁移链；R003 是最后一份签名回执，后续回执统一采用对话模式。R004 是 scope v4 审批，R005 批准首次 CI 协议返工，R006 批准受控提交传输入口，R007 批准 Git UTF-8 修复和 A→C 恢复，R008 批准 CP1252 输出/无 HOME Git 发现修复和 C→D 恢复。

## 24. 变更历史

| 版本 | 日期 | 变更摘要 | 原因 | 关联任务/审核人 |
| --- | --- | --- | --- | --- |
| v1.0 | 2026-08-19 | 建立 G0 治理基线；补齐 24 章节、受控 PASS、私有 GitHub REST、多发布单元证据、R001-R008 审批/返工链、scope v4 对话审批、首次 G0 提交闭环及两次精确 Windows CI 恢复 | 为应用初始化建立可持续维护且可追溯的前置约束 | GOV-0001 / 用户 |

本历史只描述需求文档变化；实现变化记录在任务、Git 和决策记录中。当前已有 A/C 两次非强制内容 push 与对应失败 Run，但没有 GitHub CI 通过或任务完成声明。
