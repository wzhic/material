...
| 状态 | IN_PROGRESS；R003 最终签名迁移已完成，R004 已批准 scope v4，R005/R006 已批准首次 CI 闭环与受控提交传输，R007/R009 保留两次固定历史恢复；R012 恢复内容提交的 Run 32233154643 在 Windows 因宿主可执行位摘要不一致失败，R013 已授权最小返工，R014 因合同不完整被拒绝，R015 已取代并激活；正在重新执行全部本地验证，不表示 CI 已通过或任务已完成 |
...
| 工作区摘要直接使用宿主可执行位 | 同一 Git 内容在 Windows 与 POSIX 得到不同 subject | 已跟踪文件以 Git 索引 `100644/100755` 规范化；未跟踪文件仍绑定本机模式；索引异常失败关闭 |
...
本历史只描述需求文档变化；实现变化记录在任务、Git 和决策记录中。当前 A/C/D、控制提交、后续恢复内容提交及五个失败 Run 均保留，但没有 GitHub CI 通过或任务完成声明。
