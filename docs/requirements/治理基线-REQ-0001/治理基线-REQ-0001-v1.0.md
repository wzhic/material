...
| 状态 | IN_PROGRESS；R003 最终签名迁移已完成，R004 已批准 scope v4，R005/R006 已分别批准首次 CI 闭环和受控 A/B 提交传输返工；正在重新执行全部本地验证，不表示已提交、CI 已运行或已完成 |
...
首次 G0 push 的闭环固定为：根内容提交 A 在 `GOV-0001/main` 上运行三个发布单元的全部批准检查并核对完整根快照，但只返回 `bootstrap_pending`、不写 CI PASS；随后 `taskctl` 记录 `committed_sha=A`，只形成受保护状态控制提交 B；B 再次运行全部检查，只有同时证明 A 为根提交、A 是 B 的祖先、A..B 全为受保护机器记录时才报告 `bootstrap_control_head` 并允许 GitHub REST 同步。项目 bootstrap 白名单必须精确只有 `GOV-0001`。`bootstrap-commit/bootstrap-push` 是唯一提交传输入口，固定仓库身份、SSH 443、阶段/状态和非强制 push；任何其他任务、普通内容、非根 A、非空远端、空提交、强制推送或跳过检查都不能使用该例外。
...
| AC-11 | push 或发起 PR | CI 用事件 base/head 核对已提交范围；PR detached HEAD 使用可信事件分支；首次 G0 根 A 只产生 PENDING，受保护 B 全量复验后才可同步 PASS | 待真实 GitHub run |
...
- R001、R002、R003 保留为历史迁移链；R003 是最后一份签名回执，后续回执统一采用对话模式。R004 是 scope v4 审批，R005 批准首次 CI 协议返工，R006 只批准受控 A/B 提交传输入口与完整重验。

## 24. 变更历史

| 版本 | 日期 | 变更摘要 | 原因 | 关联任务/审核人 |
| --- | --- | --- | --- | --- |
| v1.0 | 2026-08-19 | 建立 G0 治理基线；补齐 24 章节、受控 PASS、私有 GitHub REST、多发布单元证据、R001-R006 审批/返工链、scope v4 对话审批和首次 G0 A/B 提交闭环 | 为应用初始化建立可持续维护且可追溯的前置约束 | GOV-0001 / 用户 |
