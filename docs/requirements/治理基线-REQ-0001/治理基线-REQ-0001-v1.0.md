...
| 状态 | IN_PROGRESS；R003 最终签名迁移已完成，R004 已批准 scope v4，R005/R006 已批准首次 CI 闭环与受控提交传输，R007/R009 保留两次固定历史恢复；R010 恢复内容提交的 Run 32230013274 在 Windows 迁移根复验失败后，R011 已授权同范围返工，R012 已绑定 pending-content recovery proposal；正在重新执行全部本地验证，不表示 CI 已通过或任务已完成 |
...
首次 G0 push 的历史闭环固定为：根内容提交 A 在 `GOV-0001/main` 上运行三个发布单元的全部批准检查并核对完整根快照，但只返回 `bootstrap_pending`、不写 CI PASS。Run `32221854668` 的 Windows job 因 CP1252 解码 Git UTF-8 输出失败；历史 A 保留，R007 只允许 A 的固定消息、精确路径非强制子提交 C `7b18bb4c0b3cc42e078acf5351a19f6760beea0f`。Run `32223213198` 证明 Git 解码已修复，但 C 又暴露 CP1252 标准输出和无 HOME Git 发现失败；R009 取代路径白名单有遗漏的 R008，只允许 C 的固定消息、精确路径非强制子提交 D `d5e868de165ff4cd31459e13b4bb12535e0876a1`。D 的控制提交 `3e5e3e4dd9c2dcd5b76ef081913ba979dd94b762` 在 Run `32226753649` 三平台失败，因此没有 CI PASS；R010 绑定规范化 proposal 摘要，并由 `recover-committed` 核对当前 HEAD/旧 content/Run/回执、归档旧证据、清空 PASS 后回到 IN_PROGRESS。R010 恢复形成内容提交 `63b6de3b4e85b6cd6c2be8bbd43586197cec57ba`，但 Run `32230013274` 仅 Ubuntu/macOS 成功，Windows 在系统 OpenSSH 复验 R003 时失败，且尚未形成控制提交。R011 授权同范围返工；R012 绑定 schema v2 proposal，由 `recover-pending-content` 核对 pending-content 阶段、当前 HEAD、直接父子关系、历史数量和 R011 后归档 R010 活动标记。后续内容必须限于 proposal 精确路径、重新通过 local gate、成为失败 head 的直接子提交并非强制快进。以后同类恢复复用 receipt-bound 结构，禁止继续追加按 Run 硬编码的源码常量。
...
| AC-09 | 请求不可逆操作 | G0 可核验绑定当前范围、操作编号、材料摘要与有效期的用户回执；除 R010 精确 recovery proposal 的专用 `recover-committed` 消费器外，其他不可逆操作没有消费器时仍拒绝执行 | 需治理单测；R010 仅恢复治理状态，不豁免验证 |
| AC-10 | 检查批准的需求文档 | 目录、命名、24 章节、非空、适用图示和假设清单全部满足 | 需文档门禁证据 |
| AC-11 | push 或发起 PR | CI 用事件 base/head 核对已提交范围；PR detached HEAD 使用可信事件分支；首次 G0、R007/R009 历史恢复及有效 receipt-bound 恢复内容 run 只产生 PENDING，受保护控制 head 全量复验后才可同步 PASS | Run 32221854668、32223213198、32226753649、32230013274 均保留为失败证据；待 R012 恢复后的真实 run |
...
- irreversible operation 的规范化材料必须在操作前明确目标、动作、影响、恢复方案和摘要/提交；G0 回执绑定该材料摘要与有效期。R010 只为精确 receipt-bound bootstrap 恢复提供专用消费器，不能扩展为删除、强推、迁移或其他不可逆操作权限。
- 任何绑定内容变化、过期、范围修订或摘要不匹配都会使回执失效。
- R001、R002、R003 保留为历史迁移链；R003 是最后一份签名回执，后续回执统一采用对话模式。R004 是 scope v4 审批，R005 批准首次 CI 协议返工，R006 批准受控提交传输入口，R007 批准 Git UTF-8 修复和 A→C 恢复，R009 取代 R008 并批准 CP1252 输出/无 HOME Git 发现修复和 C→D 恢复；R010 绑定 Run 32226753649 后的 committed recovery proposal，R011 授权 Run 32230013274 后同范围返工，R012 绑定 pending-content recovery proposal；三者都不是验证豁免。
...
本历史只描述需求文档变化；实现变化记录在任务、Git 和决策记录中。当前 A/C/D 及控制提交历史与三个失败 Run 均保留，但没有 GitHub CI 通过或任务完成声明。
