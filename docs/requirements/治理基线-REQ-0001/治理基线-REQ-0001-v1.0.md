...
| 状态 | IN_PROGRESS；R003 最终签名迁移已完成，R004 已批准 scope v4，R005/R006 已批准首次 CI 闭环与受控提交传输，R007/R009 保留两次固定历史恢复；Run 32226753649 失败后，R010 已绑定 receipt-bound proposal 并由受控命令清空旧 PASS，正在重新执行全部本地验证，不表示 CI 已通过或任务已完成 |
...
首次 G0 push 的历史闭环固定为：根内容提交 A 在 `GOV-0001/main` 上运行三个发布单元的全部批准检查并核对完整根快照，但只返回 `bootstrap_pending`、不写 CI PASS。Run `32221854668` 的 Windows job 因 CP1252 解码 Git UTF-8 输出失败；历史 A 保留，R007 只允许 A 的固定消息、精确路径非强制子提交 C `7b18bb4c0b3cc42e078acf5351a19f6760beea0f`。Run `32223213198` 证明 Git 解码已修复，但 C 又暴露 CP1252 标准输出和无 HOME Git 发现失败；R009 取代路径白名单有遗漏的 R008，只允许 C 的固定消息、精确路径非强制子提交 D `d5e868de165ff4cd31459e13b4bb12535e0876a1`。D 的控制提交 `3e5e3e4dd9c2dcd5b76ef081913ba979dd94b762` 在 Run `32226753649` 三平台失败，因此没有 CI PASS；R010 绑定规范化 proposal 摘要，并由 `recover-committed` 核对当前 HEAD/旧 content/Run/回执、归档旧证据、清空 PASS 后回到 IN_PROGRESS。后续内容必须限于 proposal 精确路径、重新通过 local gate、成为失败控制 head 的直接子提交并非强制快进。以后同类恢复复用该 receipt-bound 结构，禁止继续追加按 Run 硬编码的源码常量。
...
| AC-11 | push 或发起 PR | CI 用事件 base/head 核对已提交范围；PR detached HEAD 使用可信事件分支；首次 G0、R007/R009 历史恢复及有效 receipt-bound 恢复内容 run 只产生 PENDING，受保护控制 head 全量复验后才可同步 PASS | Run 32221854668、32223213198、32226753649 均保留为失败证据；待 R010 恢复后的真实 run |
...
- R001、R002、R003 保留为历史迁移链；R003 是最后一份签名回执，后续回执统一采用对话模式。R004 是 scope v4 审批，R005 批准首次 CI 协议返工，R006 批准受控提交传输入口，R007 批准 Git UTF-8 修复和 A→C 恢复，R009 取代 R008 并批准 CP1252 输出/无 HOME Git 发现修复和 C→D 恢复；R010 绑定 Run 32226753649 后的一次性 receipt-bound 恢复 proposal，不是验证豁免。
