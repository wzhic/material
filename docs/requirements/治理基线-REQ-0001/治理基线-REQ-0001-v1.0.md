...
| 状态 | IN_PROGRESS；R003 最终签名迁移已完成，R004 已批准 scope v4，R005/R006 已批准首次 CI 闭环与受控提交传输，R007/R009 保留两次固定历史恢复；R015 恢复内容提交的 Run 32242354843 在 Windows 仍因 LF/CRLF 工作区字节造成摘要不一致而失败，R016 已授权最小返工，R017 已取代 R015 活动恢复合同；正在重新执行全部本地验证，不表示 CI 已通过或任务已完成 |
...
本需求只允许建立 `.codex/` Hook、`tools/governance/` 控制工具、`project-control/` 机器记录、分维度文档和 `.github/` 治理 CI。它不授权应用脚手架、应用依赖、业务代码、产品数据库迁移、合并或部署；治理快照的提交与 push 只能走 R006 建立、R007/R009 历史固定合同及 R010 receipt-bound proposal 约束的 `taskctl` 窄入口。
...
当前 R017 返工尚未提交或 push，新的 GitHub CI 和合并后验证尚未运行，状态必须保持 `PENDING`。历史失败 Run 和本轮诊断结果不能替代最终受控组合命令；只有重新执行并写入当前工作区摘要的结果才称为当前阶段证据。
...
本历史只描述需求文档变化；实现变化记录在任务、Git 和决策记录中。当前 A/C/D、控制提交、后续恢复内容提交及六个失败 Run 均保留，但没有当前 GitHub CI 通过或任务完成声明。
