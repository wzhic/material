...
| 状态 | GOV-0002 IN_PROGRESS；用户已批准简化本地工作确认点，正在补普通任务的受控分支、A/B 提交、非强制功能分支 push 与同范围恢复；当前任务尚未形成提交或 CI 证据 |
...
本需求只允许建立和维护 `.codex/` Hook、`tools/governance/` 控制工具、`project-control/` 机器记录、分维度文档和 `.github/` 治理 CI。它不授权应用脚手架、应用依赖、业务代码、产品数据库迁移、合并或部署；普通任务提交与 push 只能走 `commit-task/push-task`，GOV-0001 历史恢复仍受原有限合同约束。
