...
> 历史基线：本文保留首次 G0 根信任与故障恢复要求用于审计。普通任务现行流程已由 [治理精简-REQ-0004-v1.0](../治理精简-REQ-0004/治理精简-REQ-0004-v1.0.md) 取代；本文的 scope/code 回执、GitHub Run/Token 同步和多层状态链不得继续作为日常前置。
...
| 状态 | 历史基线；普通流程已由治理精简-REQ-0004 取代 |
...
本需求当时只允许建立和维护 `.codex/` Hook、`tools/governance/` 控制工具、`project-control/` 机器记录、分维度文档和 `.github/` 治理 CI。GOV-0001 的历史恢复仍按当时合同解释，但新普通任务不继承这些操作前置。
...
- 用户：清晰提出需求时，该请求本身就是范围决定，不再重复确认；只对验证豁免、不可逆操作、敏感数据或显著安全/成本/不兼容风险，以及最终合并、部署和发布作单独决定。范围内本地步骤和可逆技术默认值不逐项审批。
...
8. 新 scope、合并节点 code、validation waiver 和 irreversible operation 回执由 `record-conversation` 自动绑定当前治理状态；普通同范围返工不再新建 rework 回执。对话回执明确标为非密码学身份凭证，范围或专属字段变化即失效。
...
- 不执行合并、部署、发布、商店上架或不可逆迁移；当前治理任务可在完整验证后受控提交并非强制 push 功能分支。
- 不把用户延期的风险评估解释为已接受风险。
...
- 直接 Git 写操作、force push、自动合并、部署和发布；普通任务提交/push 只能走受控 CLI，合并仍需用户确认。

## 8. 前置与后置依赖

前置条件是用户已确认治理修改方向、项目为 `M + multi`、三个发布单元边界和唯一审核人权限。GOV-0001 空仓库初始化例外已经结束；普通任务必须使用独立功能分支，任何治理权限都不延伸到应用初始化。
...
- `reopen` 只处理未提交、范围不变且冻结摘要仍一致的 `LOCAL_VERIFIED` 返工；它归档并清空旧本地结果后返回 `IN_PROGRESS`，不再要求单独 `rework` 回执。
...
- `rework`：仅作为 GOV-0001 历史恢复证据保留。普通任务的未提交同范围返工由 `reopen` 直接归档；提交后同范围 CI 失败由 `recover-blocked` 追加恢复，不允许范围变化或历史改写。
- `irreversible_operation`：G0 绑定唯一操作编号、当前范围、规范化操作材料的 `target_digest` 和有效期。材料必须包含准确目标、动作、影响、不可逆后果、恢复方案和工作区摘要或提交。除 R010 精确 recovery proposal 已有专用 `recover-committed` 消费器外，G0 没有其他执行/单次消费器，因此其他回执本身不能授权工具执行；首次使用其他不可逆操作前另建治理需求。
...
治理 CI 只使用仓库 Python 标准库测试和 `reconcile`，不安装应用依赖。G0 的 push 与 PR 都按 `backend/Ubuntu`、`mac/macOS`、`win/Windows` 三个发布单元运行。所有必需步骤必须实际运行，不使用 `continue-on-error`，空测试集合也必须失败。
...
首次 G0 push 的历史闭环固定为：根内容提交 A 在 `GOV-0001/main` 上运行三个发布单元的全部批准检查并核对完整根快照，但只返回 `bootstrap_pending`、不写 CI PASS。Run `32221854668` 的 Windows job 因 CP1252 解码 Git UTF-8 输出失败；历史 A 保留，R007 只允许 A 的固定消息、精确路径非强制子提交 C `7b18bb4c0b3cc42e078acf5351a19f6760beea0f`。Run `32223213198` 证明 Git 解码已修复，但 C 又暴露 CP1252 标准输出和无 HOME Git 发现失败；R009 取代路径白名单有遗漏的 R008，只允许 C 的固定消息、精确路径非强制子提交 D `d5e868de165ff4cd31459e13b4bb12535e0876a1`。D 的控制提交 `3e5e3e4dd9c2dcd5b76ef081913ba979dd94b762` 在 Run `32226753649` 三平台失败，因此没有 CI PASS；R010 绑定规范化 proposal 摘要，并由 `recover-committed` 核对当前 HEAD/旧 content/Run/回执、归档旧证据、清空 PASS 后回到 IN_PROGRESS。R010 恢复形成内容提交 `63b6de3b4e85b6cd6c2be8bbd43586197cec57ba`，但 Run `32230013274` 仅 Ubuntu/macOS 成功，Windows 在系统 OpenSSH 复验 R003 时失败，且尚未形成控制提交。R011 授权同范围返工；R012 绑定 schema v2 proposal，由 `recover-pending-content` 核对 pending-content 阶段、当前 HEAD、直接父子关系、历史数量和 R011 后归档 R010 活动标记。R012 恢复形成内容提交 `0b27ffaa178dd3ffc1ff0d53b899099126839e07`，Run `32233154643` 的 Ubuntu/macOS 成功，但 Windows 因已跟踪文件摘要读取宿主 POSIX 可执行位而出现 local subject 不匹配。R013 授权最小返工；R014 因缺完整安全不变量被门禁拒绝并保留，R015 用固定摘要、提交消息、11 个允许路径和完整安全不变量取代并激活。摘要对已跟踪普通文件使用 Git 索引 `100644/100755` 规范化，对未跟踪文件保留本机模式；后续内容仍须重新通过 local gate、成为失败 head 的直接子提交并非强制快进。以后同类恢复复用 receipt-bound 结构，禁止继续追加按 Run 硬编码的源码常量。
...
stateDiagram-v2
    ...
    LOCAL_VERIFIED --> IN_PROGRESS: 同范围 taskctl reopen
    LOCAL_VERIFIED --> COMMITTED: taskctl commit-task --manifest 精确记录内容 A / 控制 B
...
flowchart TD
    ...
    L --> M[taskctl 受控提交并非强制 push 功能分支]
...
流程图不能代替异常规则：真正高风险节点缺明确用户决定、R003 迁移根损坏、范围变化、错误分支、状态损坏、runner 改动工作区、GitHub 网络/凭据缺失、事件 SHA 不可信、测试未运行或证据阶段不符都必须进入阻断。清晰的用户需求可直接形成范围回执，不再额外索取一次确认；验证豁免、不可逆操作、敏感数据或显著安全/成本/不兼容风险及合并/部署/发布仍需明确决定。
...
- G0 bootstrap 与控制提交的 push、PR 都运行 Ubuntu、macOS、Windows；后续普通需求若要按成本缩减矩阵，必须由独立需求审核，不能复用 G0 例外。
...
产品发布规则见[发布与回退](../../operations/发布与回退-OPS-0001-v1.0.md)：三个发布单元独立版本、自动验证、真实环境验收、发布和回退；跨单元变化使用协调任务和逐单元证据。任何删除历史、破坏性迁移、强制覆盖或不可恢复操作都必须先有用户明确对话决定及有效 `irreversible_operation` 回执；除 R010 精确治理恢复已有专用消费器外，G0 尚无其他执行/单次消费器，因此首次实际执行其他不可逆操作前还必须通过独立治理需求补齐并验证对应通道。
...
| AC-09 | 请求不可逆操作 | G0 可核验绑定当前范围、操作编号、材料摘要与有效期的用户回执；只有已实现并验证的 receipt-bound `recover-committed` / `recover-pending-content` 可消费对应恢复提案，其他不可逆操作没有消费器时仍拒绝执行 | 需治理单测；R010/R012/R015 只恢复治理状态，不豁免验证；R014 已被门禁拒绝 |
| AC-10 | 检查批准的需求文档 | 目录、命名、24 章节、非空、适用图示和假设清单全部满足 | 需文档门禁证据 |
| AC-11 | push 或发起 PR | CI 用事件 base/head 核对已提交范围；PR detached HEAD 使用可信事件分支；首次 G0、R007/R009 历史恢复及有效 receipt-bound 恢复内容 run 只产生 PENDING，受保护控制 head 全量复验后才可同步 PASS | Run 32221854668、32223213198、32226753649、32230013274、32233154643 均保留为失败证据；待 R015 恢复后的真实 run |
...
AC-11、AC-12 和 AC-17 的真实运行部分在 GOV-0002 尚未提交和 push 时必须保持 `PENDING`，不得用 workflow 文件存在、本地测试、URL 或模拟 REST 响应代替。AC-15 的 R003 真实迁移已完成；后续关键节点对话回执仍只能在用户明确回复后记录。
...
当前 GOV-0002 尚未形成内容/控制提交或新的 GitHub CI，相关状态必须保持 `PENDING`。GOV-0001 历史失败 Run 只作回归材料，不能替代当前任务的受控命令和准确工作区/提交证据。
...
| 工作区摘要读取宿主可执行位或原始换行字节 | 同一 Git 内容在 Windows 与 POSIX 得到不同 subject | 可执行类别取 Git 索引模式，内容取 Git clean-filter blob；LF/CRLF 等价但真实内容变化失效；Git 异常失败关闭 |
| 跨平台 CI 成本过高 | 单人维护负担 | G0 push/PR 保持三平台；后续需求若缩减矩阵须独立审核，不能牺牲发布单元证据 |

产品层面的最坏后果目前无法评估，用户已明确延期。它阻断相关技术选型和业务开发，不阻断仅建立 G0 控制层；该处理不代表用户接受未知风险。

## 23. 审核与回执

用户已在对话中确认 GOV-0002 修改方向和边界：范围内本地文件、测试、分支、完整验证后的受控提交和非强制功能分支 push 可持续执行；该决定不授权合并、部署、发布、不可逆操作、业务代码或应用初始化。机器回执是否有效只由 `reviewctl verify` 和 `reconcile` 判断。
...
- scope 审核包由本需求、当前任务、范围、验证和回退计划组成；任务进入 `REVIEW_PENDING` 后交用户确认。
...
- 普通任务的未提交同范围返工不再生成 rework 回执，只有 `taskctl reopen` 可以在冻结摘要仍一致时归档旧证据并恢复写入；历史 rework 回执继续保留。
- irreversible operation 的规范化材料必须在操作前明确目标、动作、影响、恢复方案和摘要/提交；G0 回执绑定该材料摘要与有效期。R010/R012/R015/R017 只为各自精确 receipt-bound bootstrap 恢复提供专用消费，不能扩展为删除、强推、迁移或其他不可逆操作权限；被拒绝的 R014 不授予执行权。
- 任何绑定内容变化、过期、范围修订或摘要不匹配都会使回执失效。
- R001、R002、R003 保留为历史迁移链；R003 是最后一份签名回执，后续回执统一采用对话模式。R004 是 scope v4 审批，R005 批准首次 CI 协议返工，R006 批准受控提交传输入口，R007 批准 Git UTF-8 修复和 A→C 恢复，R009 取代 R008 并批准 CP1252 输出/无 HOME Git 发现修复和 C→D 恢复；R010 绑定 Run 32226753649 后的 committed recovery proposal，R011 授权 Run 32230013274 后同范围返工，R012 绑定 pending-content recovery proposal；R013 授权 Run 32233154643 后的最小可执行模式返工，R014 因合同不完整被拒绝，R015 取代 R014；R016 授权 Run 32242354843 后的 Git blob 规范化返工，R017 绑定新的 pending-content recovery proposal。它们都不是验证豁免。

## 24. 变更历史

| 版本 | 日期 | 变更摘要 | 原因 | 关联任务/审核人 |
| --- | --- | --- | --- | --- |
| v1.0 | 2026-08-19 | 建立 G0 治理基线；补齐 24 章节、受控 PASS、私有 GitHub REST、多发布单元证据、scope v4 对话审批、历史 R007/R009 固定恢复与 R010 receipt-bound 提交后恢复 | 为应用初始化建立可持续维护且可追溯的前置约束 | GOV-0001 / 用户 |
| v1.0 治理修订 | 2026-08-19 | 清晰需求不重复确认；普通任务使用精确变更清单持续本地工作、受控 A/B 提交、非强制 push 与同范围返工；仅高风险和最终交付节点确认 | 降低单人维护摩擦，同时防止无关本地改动被带入提交 | GOV-0002 / 用户 |

本历史只描述需求文档变化；实现变化记录在任务、Git 和决策记录中。GOV-0001 的 A/C/D、控制提交、后续恢复内容及失败 Run 均保留，但没有 GOV-0002 当前 GitHub CI 通过或任务完成声明。
