...
| Windows CI 在 `learned-runtime-bundle.test.ts` 创建链接时失败 | hosted runner 没有文件 symlink 提升权限，而不是 verifier 接受了链接 | Windows 回归使用目录 junction 真实验证链接拒绝；POSIX 保留文件 symlink，不能删除或跳过链接安全断言 |
| Windows 全量桌面测试把 `learned-runtime-bundle.test.ts` 识别为最小失败集合，但专项 runtime 测试通过 | junction 在 verifier 枚举后留给通用递归清理，繁忙全量套件中可能出现句柄释放时序差异 | 断言 junction 已真实创建；验证拒绝后在 `finally` 中带有限重试显式移除，再执行根目录递归清理；全量与专项测试必须同时通过 |
| Windows 全量桌面测试在显式清理 junction 后仍把 `learned-runtime-bundle.test.ts` 识别为相同最小失败集合 | 测试夹具把目标声明为 `darwin`，使 Windows 文件被错误套用 POSIX 可执行位合同 | 夹具在 Windows 声明 `win32`、其他受控测试端声明 `darwin`；继续验证同一 manifest、文件集、hash、目标不匹配和链接失败关闭，不跳过断言 |
